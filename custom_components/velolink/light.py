    ...
    def _handle_new_node(node):
        if node.kind == NODE_KIND_PWM:
            for ch in range(node.channels):
                ...
                created.add(uid)
                async_add_entities(
                    [VelolinkPWMEntity(hub, entry.entry_id, hub, node, ch)]
                )
...
class VelolinkPWMEntity(LightEntity):
    ...
    @property
    def unique_id(self):
        return f"{self._bus_id}-{self._addr}-pwm-{self._ch}"

    @property
    def name(self):
        return f"PWM {self._ch} [{self._addr}]"
    ...
    async def async_turn_on(self, **kwargs):
        ...
        await self._hub.async_set_pwm(
            self._bus_id, self._addr, self._ch, self._brightness
        )
