...
class VelolinkNode:
    ...
    def __init__(self, bus_id: BusId, address: Addr, kind: str, channels: int, model: str = "Unknown"):
        self.bus_id = bus_id
...
class TcpGateway:
    ...
    async def _read_loop(self) -> None:
        ...
        while self._running:
            ...
            if not data: raise ConnectionError("TCP closed")
            buffer.extend(data)
            while True:
    ...
    def _extract_tcp_packet(self, buffer: bytearray) -> Optional[bytes]:
        ...
        while len(buffer) >= min_len:
            if buffer[0:magic_len] != TCP_HEADER_MAGIC: buffer.pop(0); continue
    ...
    def _process_tcp_packet(self, packet: bytes) -> None:
        magic_len = len(TCP_HEADER_MAGIC)
        if packet[0:magic_len] != TCP_HEADER_MAGIC: return
        if packet[magic_len] != TCP_PROTOCOL_VERSION: return
        ...
        if bus_byte == 0x01: bus_id_str = "bus1"
        elif bus_byte == 0x02: bus_id_str = "bus2"
        elif bus_byte == 0x03: bus_id_str = "bus_wifi"
        else: return
    ...
    async def async_write_frame(self, bus_id: BusId, frame: bytes) -> None:
        if not self._writer: raise RuntimeError("TCP not connected")
        async with self._writer_lock:
            magic = TCP_HEADER_MAGIC
            version = bytes([TCP_PROTOCOL_VERSION])
            
            # Mapowanie Python ID na bajt protokołu
            if bus_id == "bus1": bus_byte = bytes([0x01])
            elif bus_id == "bus2": bus_byte = bytes([0x02])
            elif bus_id == "bus_wifi": bus_byte = bytes([0x03])
            else: raise ValueError("Unknown Bus ID")
...
class VelolinkHub:
    ...
    async def async_set_output(self, bus_id: BusId, addr: Addr, ch: Channel, on: bool) -> None:
        payload = bytes([ch & 0xFF, 1 if on else 0])
        frame = self._build_frame(addr, FunctionCode.SET_OUTPUT, payload)
        await self._gateway.async_write_frame(bus_id, frame)

    async def async_set_pwm(self, bus_id: BusId, addr: Addr, ch: Channel, value: int) -> None:
    ...
    def subscribe_input(self, bus_id, addr, ch, cb): return self._add_sub(self._subs_input, bus_id, addr, ch, cb)
    def subscribe_output(self, bus_id, addr, ch, cb): return self._add_sub(self._subs_output, bus_id, addr, ch, cb)
    def subscribe_pwm(self, bus_id, addr, ch, cb): return self._add_sub(self._subs_pwm, bus_id, addr, ch, cb)
    def subscribe_analog(self, bus_id, addr, ch, cb): return self._add_sub(self._subs_analog, bus_id, addr, ch, cb)

    def _add_sub(self, bucket, bus_id, addr, ch, cb):
        key = (bus_id, addr, ch)
        bucket.setdefault(key, []).append(cb)
        def unsub(): lst = bucket.get(key, []); lst.remove(cb) if cb in lst else None
        return unsub
    ...
    def _parse_frame(self, frame: bytes) -> dict:
        ...
        if len(frame) < 8 or frame[0:pre_len] != FRAME_PREAMBLE: raise ValueError("bad preamble")
        
        length = frame[5]
        if len(frame) != 6 + length + 2: raise ValueError("length mismatch")

        body = frame[2:-2]
        crc_recv = frame[-2] | (frame[-1] << 8)
        crc_calc = TcpGateway._crc16(body)
        if crc_recv != crc_calc: raise ValueError("CRC error")
        ...
        if func == FunctionCode.HELLO:
            ...
            channels = payload[1]
            model = payload[8:8+payload[7]].decode("ascii") if len(payload) > 8 else "Unknown"
            return {"type": "HELLO", "addr": addr, "kind": kind, "channels": channels, "model": model}
        
        if func == FunctionCode.BUTTON_EVENT:
            return {"type": "BUTTON_EVENT", "addr": addr, "ch": payload[0], "value": payload[1]}
