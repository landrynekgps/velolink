    ...
    def _handle_new_node(node):
        if node.kind == NODE_KIND_OUTPUT:
            for ch in range(node.channels):
                ...
                created.add(uid)
                async_add_entities(
                    [VelolinkOutEntity(hub, entry.entry_id, hub, node, ch)]
                )
...
class VelolinkOutEntity(SwitchEntity):
    ...
    @property
    def unique_id(self):
        return f"{self._bus_id}-{self._addr}-out-{self._ch}"

    @property
    def name(self):
        return f"Out {self._ch} [{self._addr}]"
    ...
    async def async_turn_on(self, **kwargs):
        await self._hub.async_set_output(self._bus_id, self._addr, self._ch, True)

    async def async_turn_off(self, **kwargs):
        await self._hub.async_set_output(self._bus_id, self._addr, self._ch, False)
