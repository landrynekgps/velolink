    ...
    def _handle_new_node(node):
        if node.kind == NODE_KIND_OUTPUT:
            for ch in range(node.channels):
                ...
                created.add(uid)
                async_add_entities([VelolinkBtnEntity(hub, entry.entry_id, hub, node, ch)])
...
class VelolinkBtnEntity(BinarySensorEntity):
    ...
    @property
    def unique_id(self): return f"{self._bus_id}-{self._addr}-btn-{self._ch}"
    @property
    def name(self): return f"Btn {self._ch} [{self._addr}]"
    @property
    ...
    async def async_added_to_hass(self):
        ...
        self._unsub = self._hub.subscribe_input(self._bus_id, self._addr, self._ch, _on_change)
